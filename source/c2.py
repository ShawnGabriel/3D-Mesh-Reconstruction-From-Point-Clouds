...
DO=0.3
steps = [1,1]
outdir = 'PATH/TO/YOU/DIR' # Your output dir
tasks = [0, 1, 2]
IT=1
IV=2
WT=1
import random
import unet_variants
import numpy as np
    ...
    while True:
        ...
        for i in range(BS):
            ...
            if (random.random()>.999) | show_now :
                ...
                plt.tight_layout()
                plt.savefig( '/PATH/TO/YOUR/batches_%d_IR%d_EN%d_sl%d_mask.png'%(id_trn, IR, EN, i ) ) # Replace with your results dir
                plt.close('all')
                o=in_[ 0, :, :]
                o=255*(o/(o.max()+1e-7))
                plt.imshow( o.astype(int) )
                plt.tight_layout()
                plt.savefig( '/PATH/TO/YOUR/batches_%d_IR%d_EN%d_sl%d_img.png'%(id_trn, IR, EN, i ) ) # Replace with your results dir
            try:
...
class CyclicalLearningRate(tf.keras.optimizers.schedules.LearningRateSchedule):
    def __init__(self, initial_lr, max_lr, step_size, scale_fn=lambda x: 1/(2.**(x-1))):
    ...
    def __call__(self, step):
        ...
        return lr * self.scale_fn(cycle)
    def get_config(self):
        return {
            "initial_lr": self.initial_lr,
            "max_lr": self.max_lr,
            "step_size": self.step_size,
            "scale_fn": self.scale_fn.__name__}
    def from_config(cls, config):
        config["scale_fn"] = eval(config["scale_fn"])  # Reconstruct scale_fn from name
        return cls(**config)
...
if CR==2:
    ...
    clr = CyclicalLearningRate(LR, 2*LR, step_size=2*steps[0])
...
for MODE in tasks:
    if MODE ==0:
        ...
        # ====================== get prefix ==============================================================================
        def get_prefix():
            # Construct a unique subdirectory based on model parameters
        ...
        model_checkpoint_callback = tf.keras.callbacks.ModelCheckpoint(
            monitor='DirectedHD',
            filepath=os.path.join(prefix, 'ckpt.keras'),  # Use os.path.join for path construction
            mode='auto',
            save_best_only=True,
        ...
        # Remove use_multiprocessing argument
        hist = model.fit(
            trn_gen,
